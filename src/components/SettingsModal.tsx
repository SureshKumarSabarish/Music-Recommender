...
interface SettingsModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (keys: { geminiKey: string, spotifyClientId: string, spotifyClientSecret: string }) => void;
}
...
export function SettingsModal({ isOpen, onClose, onSave }: SettingsModalProps) {
  ...
  const handleSave = () => {
    ...
    const trimmedClientId = spotifyClientId.trim();
    const trimmedClientSecret = spotifyClientSecret.trim();

    localStorage.setItem('gemini_api_key', trimmedGemini);
    localStorage.setItem('spotify_client_id', trimmedClientId);
    localStorage.setItem('spotify_client_secret', trimmedClientSecret);
    
    onSave({
      geminiKey: trimmedGemini,
      spotifyClientId: trimmedClientId,
      spotifyClientSecret: trimmedClientSecret
    });
